...
After making changes, execute the following to check for correct formatting. This runs the linting scripts `format:toc`, `format:prettier` and `format:sh`. Check the output to see if there are any formatting issues that need manual correction:
...
| `npm run format:sh`             | Reformats shell scripts                 |
